	...
	{	
		...
		*/
		public void GoRagdoll (string reason){
			...
			*/
			...
			if(GetComponent<DynamicRagdoll.Demo.AIControl>()){
				GetComponent<DynamicRagdoll.Demo.AIControl>().shouldAddScore = true;
				...
				Camera.main.GetComponent<DynamicRagdoll.Demo.CameraHandler>().SetTarget(true);
				GameObject.FindWithTag("Sound").GetComponent<AudioSource>().Stop();
				StartCoroutine(GoSlowMo());
				disableGetUp = true;
			}
		}


		IEnumerator GoSlowMo(){
			Time.timeScale = 0.3f;
		...
		*/
		void StartGetUp () {
			...
			Camera.main.GetComponent<DynamicRagdoll.Demo.CameraHandler>().SetTarget(false);
